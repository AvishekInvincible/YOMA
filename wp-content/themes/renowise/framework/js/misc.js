...
(function ($) {
  function initFancySelect() {
    if (typeof $.fn.fancySelect === "function") {
      $(
        ".no-touch .btSidebar select:not(.dropdown_product_cat), .no-touch #btSettingsPanelContent select, .no-touch .wpcf7-form select:not([multiple]):not(.skipFancy)"
      ).fancySelect();
    }
  }
  ...
  function boldthemes_onCatChange() {
    if (
      boldthemes_dropdown.options[boldthemes_dropdown.selectedIndex].value > 0
    ) {
      ...
      if (boldthemes_dropdown !== null)
        location.href =
          home_url +
          "?cat=" +
          boldthemes_dropdown.options[boldthemes_dropdown.selectedIndex].value;
    }
  }
